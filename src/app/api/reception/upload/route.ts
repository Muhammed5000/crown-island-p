...
/**
 * Reception guest-ID / payment-proof upload — accepts a single multipart `file`
 * (image only), stores it in the PRIVATE secure root (served only by the auth-
 * gated `/api/secure-media/...` route), records a `Media` row, and — on the local
 * venue node — durably queues the bytes for a verified push to the online master.
 * Gated to reception-authorised staff (STAFF / admin tiers, never SECURITY).
 */
...
export async function POST(req: NextRequest) {
  ...
  if (!user || !canAccessReception(user.role)) return err('unauthorized', 401);

  // DoS containment: cap uploads per staff member per minute (generous enough
  // for bulk per-guest ID capture; stops a runaway loop / compromised account).
  const rate = checkUploadRate(`reception-upload:${user.id}`);
  if (!rate.ok) {
  ...
  // Payment proofs + guest IDs are SENSITIVE: store them in the private root and
  // hand back a `/api/secure-media/...` URL served only by the auth-gated route,
  // never as a world-readable static asset under public/.
  ...
  // written file (no orphaned PII bytes) and 500 so the desk retries.
  ...
  try {
    ...
    log.error('reception upload failed to record upload', errFields(e));
  ...
  // the bytes before the reception booking (committed on online) references this
  // URL. If it fails we still return ok — the queued row drains on a later tick;
  // ID capture must keep working while online is briefly unreachable.
  const pushed = await pushFileToOnline(url, mime, buffer, user.id);
  if (pushed.ok && queued) {
    // Race-safe vs a concurrent worker drain of the same row (gated on
    // still-pending) so the worker doesn't re-push what we just confirmed.
