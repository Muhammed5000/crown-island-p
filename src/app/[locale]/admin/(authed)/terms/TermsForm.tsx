...
import { updateTermsAction } from '@/features/admin/settings-actions';

interface Props {
  initialTermsEn: string;
  initialTermsAr: string;
}
 ...
 */
export function TermsForm({ initialTermsEn, initialTermsAr }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await updateTermsAction(formData);
      if (res.ok) {
        router.refresh();
      } else {
        alert('Failed to update terms. Check the console.');
      }
    });
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        ...
        <CardBody>
          <textarea
            name="termsAr"
            defaultValue={initialTermsAr}
            required
            dir="rtl"
            rows={15}
            className="w-full rounded-2xl border border-border/40 bg-input p-4 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent"
            placeholder="أدخل الشروط والأحكام الكاملة هنا..."
          />
        </CardBody>
      </Card>
