...
/** Beaches tab — a simple grid of only the NORMAL (beach) categories. */
export default async function BookingBeachesPage({
  params,
}: {
  ...
  const [categories, tNav] = await Promise.all([
    listActiveCategoryCards('NORMAL'),
    getTranslations('nav'),
  ]);

  return <CategoryListView locale={locale} categories={categories} title={tNav('beaches')} />;
}
